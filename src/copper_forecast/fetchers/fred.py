...
def _fred_api(series_id: str, api_key: str, lookback_days: int) -> pd.DataFrame:
    ...
    for attempt in range(1, _FRED_API_RETRIES + 1):
        try:
            ...
            last_exc = exc
            if attempt < _FRED_API_RETRIES:
                time.sleep(_FRED_API_RETRY_DELAY_SEC * attempt)
    assert last_exc is not None
    raise last_exc


def _load_fred_frame(
    series_id: str,
    indicator: str,
    lookback_days: int,
    result: FetchResult,
    ...
    if api_key:
        try:
            ...
            result.errors.append(
                f"fred:{indicator}: API failed after {_FRED_API_RETRIES} retries: {exc}"
            )
            return pd.DataFrame()
...
def fetch_fred_series(
    ...
    result = FetchResult()
    frame = _load_fred_frame(series_id, indicator, lookback_days, result)
    if frame.empty:
        if not any(indicator in err for err in result.errors):
...
def korea_exports_yoy(lookback_months: int = 36) -> FetchResult:
    ...
    result = FetchResult()
    frame = _load_fred_frame("XTEXVA01KRM667S", "korea_exports_yoy", lookback_months * 31, result)
    if frame.empty:
        return result
    try:
