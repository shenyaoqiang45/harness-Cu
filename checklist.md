...
4. **截至 2026-07-31**：本轮核 White House Presidential Actions 抓取超时；无可靠新精炼铜 Proclamation 文本前维持 `confidence B`（最近已确认铜相关仍为 2026-06-01）。
...
| [ ] | P0 | **07-31** | 中国官方制造业 PMI（7 月，统计局，月末） | 中国需求 | **已核、待发布**：官网预告今日 **09:30**；`/sj/zxfb/202607/` 页面尚未出（404）；**暂不录入**，发布后录 `china_pmi` / `china_new_orders_pmi` 再 `cli run` |
| [ ] | P3 | **07-31** | 中国财新制造业 PMI（7 月，若发布） | —（交叉验证） | 已核：待官方 PMI 后再对；未发布则跳过 |

### 2026-08

| 状态 | 优先级 | 日期 | 事件 | 影响模块 | 触发动作 |
|:---:|---|---|---|---|---|
| [ ] | P1 | **08-01** | 美国 ISM 制造业 PMI（7 月） | 全球制造业 | 录 `us_ism_manufacturing`；`cli run`（ISM 日历：Manufacturing PMI® July → 8/1） |
| [ ] | P1 | **08-01** | 韩国出口同比（7 月） | 全球制造业 | 录 `korea_exports_yoy`；注意基数切换异常；`cli run` |
| [ ] | P2 | **08-02** | **7 月窗口收口 · 月报对照** | 全模块 | 对比 `live_2026-07-01_*` vs `live_2026-07-31_*`；未勾七月项转入归档或结转 |
| [ ] | P1 | **08-03** | 全球制造业 PMI（7 月终值，S&P Global / JPMorgan） | 全球制造业 | 录 `global_manufacturing_pmi`；`cli run`（BMO 日历：S&P Global Mfg PMI July F ≈ 8/3） |
