...
def test_global_pmi_momentum_kept_when_same_source():
    grouped = {
        "global_manufacturing_pmi": [
            _row("global_manufacturing_pmi", date(2026, 5, 1), 52.7, source="S&P Global"),
            _row("global_manufacturing_pmi", date(2026, 6, 1), 52.2, source="S&P Global"),
        ]
    }
    mod = score_global_cycle(grouped)
    moms = [s for s in mod.signals if s.name == "global_manufacturing_pmi_mom"]
    assert len(moms) == 1
    assert moms[0].score == -1.0  # 52.7 -> 52.2 is a modest easing
